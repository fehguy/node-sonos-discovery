...
SoundServiceErrorWatcher.prototype.recordError = function (name) {
    ...
    if (typeof errorValue === "undefined") {
        SoundServiceErrorWatcher.errorMap[host] = date;
...
function Subscriber(subscribeUrl, notificationUrl, _subscriptionInterval, _retryInterval) {
    ...
    function subscribe() {
        ...
        request({
            ...
            var lastError = new SoundServiceErrorWatcher().recordError(subscribeUrl);
            var duration = new Date().getTime() - lastError.getTime();
            logger.warn(`resubscribing to sid ${subscribeUrl} failed ${duration} MS ago`);
            sid = null;
            timer = setTimeout(subscribe, retryInterval);

            if ((new Date().getTime() - lastError.getTime()) > 15000) {
